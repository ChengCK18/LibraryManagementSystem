...
def login_check():
	...
	while True:
		if  user_id[0] == id and user_password[0] == password :
			...
			tkMessageBox.showinfo("Login"," The student ID or Password that you have entered is incorrect.Please reenter")
			return(login_in)
	if verification==1:
		...
		tkMessageBox.showinfo("Login","Greetings!Feedbacks from users are listed here")
		feedback_read()
...
def Main_Menu():
	...
	but_font = tkFont.Font(family='Helvetica', size=15, weight=tkFont.BOLD)
...
def borrow_check():
	...
	if (bbook in bookborrow):
		...
		if bookborrow[Avail] == str("Unavailable\n"):
			tkMessageBox.showinfo("Borrow","This book is currently unavailable, please select another book")
			borrow_menu.lift()
			u = 1
		else:
			...
			tkMessageBox.showinfo("Borrow","The book you have selected has been successfully borrowed. Please return it by:" +'\n'+ str(enddate) )
			...
			Done2=tkMessageBox.askyesno("Borrow","Do you want to borrow another book?")
			if Done2==True:
				borrow_menu.destroy()
				borrow_in()
			else:
		...
		tkMessageBox.showinfo("Borrow","The book that you had entered is not in our database,sorry,please enter a different book")
		borrow_menu.lift()
...
def return_check():	
	...
	if (bbook in bookborrow)and (bbook in record_verification):
		...
		if bookborrow[Avail] == str("Available\n"):
			tkMessageBox.showinfo("This book has been returned, please select another book")				

		else:
			bookborrow[Avail] = str("Available\n")
			with open("Bookdata.txt",'w') as file:
				file.writelines(bookborrow)			
			tkMessageBox.showinfo("Return","The book you have selected has been successfully returned on"+'\n'+str(date))
			...
			Done3=tkMessageBox.askyesno("Return","Do you want to return another book?")
			if Done3==True:
				return_menu.destroy()
				return_in()
			else:
		...
		tkMessageBox.showinfo("Return","The book that you had entered is invalid.Please reenter a different book")
		return_menu.lift()
...
def shop_in():
	...
	tkMessageBox.showinfo("Shop","Get 10% off from your total purchase when you purchase more than RM50")
	shop_menu.lift()
...
def shop_in_check(): #This module is to check if the user's input is a valid integer or not
	...
	try:
		...
		tkMessageBox.showinfo("Shop","One or more of your entries is not a valid input.Please reenter :D ")
		shop_menu.lift()
		return(shop_in)
...
def shop_check():
	...
	if itemleft1<0:
		...
		price1=0
		tkMessageBox.showinfo("Shop","Sorry it seems like we don't have enough MMU T-Shirt in stock.We apologize for the inconvenience caused")
		shop_menu.lift()
		return(shop_in)
	...
	if itemleft2<0:
		...
		price2=0
		tkMessageBox.showinfo("Shop","Sorry it seems like we don't have enough MMU lanyard in stock.We apologize for the inconvenience caused")
		shop_menu.lift()
		return(shop_in)							
	...
	if itemleft3<0:
		...
		price3=0
		tkMessageBox.showinfo("Shop","Sorry it seems like we don't have enough MMU Student Handbook in stock.We apologize for the inconvenience caused")
		shop_menu.lift()
		return(shop_in)							
	...
	if itemleft4<0:
		...
		price4=0
		tkMessageBox.showinfo("Shop","Sorry it seems like we don't have enough MMU Student Card in stock.We apologize for the inconvenience caused")
		shop_menu.lift()
		return(shop_in)					
	...
	if itemleft5<0:
		...
		price5=0
		tkMessageBox.showinfo("Shop","Sorry it seems like we don't have enough MMU Academic Calendar in stock.We apologize for the inconvenience caused")
		shop_menu.lift()
		return(shop_in)						
	totalprice=totalprice+price1+price2+price3+price4+price5
	if totalprice>50:
		totalprice=totalprice-(totalprice*1/10)	
	Done=tkMessageBox.askyesno("Shop","Do you wish to checkout?")
	if Done== True:
		pass
	else: 
		tkMessageBox.showinfo("Shop","Please fill the quantity(s) again.")
		totalprice=0
		itemleft1=itemleft1+quantity1
		itemleft2=itemleft2+quantity2
		itemleft3=itemleft3+quantity3
	...
	tkMessageBox.showinfo("Shop","Your bill and item(s) is ready."+"Payment= RM"+totalprice+" .Kindly collect your items at our service counter during working hours.Thank you.")
	shop_menu.destroy()
...
def search_check():
	...
	if (search_word in booklist):
		...
		tkMessageBox.showinfo("Search","It is in our database!")
		...
		tkMessageBox.showinfo("Search","Sorry,this book does not exist in our database")
...
def feedback_check():
	...
	if len(feedback_bar.get())==0:
		tkMessageBox.showinfo("Feedback","You did not type anything O_O")
		feedback_menu.lift()
		return(feedback_in)
	else:
		tkMessageBox.showinfo("Feedback","Thank you for your valuable feedback! >_<")
